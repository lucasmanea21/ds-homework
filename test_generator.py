...
def write_all_tests_to_file(filename, test_cases):
    with open(filename, "w") as f:
        ...
        for test_type, N, MAX, data in test_cases:
            f.write(f"# {test_type}\n")
            f.write(f"{N} {MAX}\n")
            for num in data:
                f.write(f"{num}\n")
...
for n in sizes:
    ...
    test_cases.append(("equal values", n, max_val, generate_all_equal(n)))
    test_cases.append(("many duplicates", n, max_val, generate_many_duplicates(n)))


# floats
test_cases.append(("random floats", 10_000, 1000, generate_random_floats(10_000, 1000)))
test_cases.append(("random floats", 100_000, 1000, generate_random_floats(100_000, 1000)))
